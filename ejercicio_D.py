...
class AuthenticationServer:
    ...
    def issue_tgt(self, user):
        ...
        f_exp = datetime.now() + timedelta(seconds=4)
        tgt_data = f"{user}|{time.time()}|{f_exp}|TGS".encode()
...
class TicketGrantingServer:
    ...
    def issue_service_ticket(self, tgt, service):
        try:
            ...
            user, timestamp, ti_exp, realm = tgt_data.decode().split('|')
            f_exp = datetime.strptime(ti_exp, "%Y-%m-%d %H:%M:%S.%f")
